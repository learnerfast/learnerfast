    ...
    if (activeTab === 'course-page-layout' && iframeRef.current && !isLoadingPreview && selectedWebsites.length > 0) {
      ...
      const timer = setTimeout(() => {
        try {
          ...
          script.textContent = `(function(){const c=${JSON.stringify(courseData)};const t=document.querySelector('.course-title, h1');if(t)t.textContent=c.title;const i=document.querySelector('.course-image, img[alt*="course"], img[alt*="Course"]');if(i&&c.image)i.src=c.image;const d=document.querySelector('.course-description, .description, p.text-lg');if(d)d.textContent=c.description||'';const p=document.querySelector('.course-price, .price');const cp=p?.nextElementSibling;if(p){if(c.access_type==='free'||c.price===0){p.textContent='Free';if(cp?.classList.contains('line-through'))cp.style.display='none'}else if(c.access_type==='coming-soon'){p.textContent='Coming Soon';if(cp)cp.style.display='none'}else if(c.access_type==='enrollment-closed'){p.textContent='Enrollment Closed';if(cp)cp.style.display='none'}else{p.textContent='₹'+c.price;if(cp?.classList.contains('line-through')){if(c.showComparePrice&&c.comparePrice>c.price){cp.textContent='₹'+c.comparePrice;cp.style.display='block'}else{cp.style.display='none'}}}}const inc=document.querySelector('.course-includes, ul');if(inc){const items=c.label?c.label.split('\\n').filter(i=>i.trim()):[];if(c.showCourseIncludes&&items.length>0){inc.innerHTML=items.map(item=>'<li class="flex items-center gap-3"><span class="material-symbols-outlined text-base text-primary">check_circle</span><span>'+item+'</span></li>').join('')}else if(!c.showCourseIncludes){const s=inc.closest('section');if(s)s.style.display='none'}}const ls=document.querySelector('section h3, section h2');if(ls&&ls.textContent.includes("What you'll learn")){const items=c.whatYouLearn?c.whatYouLearn.split('\\n').filter(i=>i.trim()):[];if(c.showWhatYouLearn&&items.length>0){const g=ls.nextElementSibling;if(g?.classList.contains('grid')){g.innerHTML=items.map(item=>'<div class="flex items-start gap-4 p-4 rounded-lg bg-background-light dark:bg-gray-800/50"><span class="material-symbols-outlined text-primary mt-1">check_circle</span><p class="font-medium text-gray-800 dark:text-gray-200">'+item+'</p></div>').join('')}}else if(!c.showWhatYouLearn){const s=ls.closest('section');if(s)s.style.display='none'}}if(!c.showInstructor){document.querySelectorAll('section').forEach(s=>{const h=s.querySelector('h3, h2');if(h?.textContent.toLowerCase().includes('instructor'))s.style.display='none'})}else{const n=document.querySelector('.instructor-name');if(n&&c.instructorName)n.textContent=c.instructorName;const ti=document.querySelector('.instructor-title');if(ti&&c.instructorTitle)ti.textContent=c.instructorTitle;const b=document.querySelector('.instructor-bio');if(b&&c.instructorBio)b.textContent=c.instructorBio}document.querySelectorAll('a, button').forEach(el=>{const txt=el.textContent.trim().toLowerCase();if(txt.includes('back')||txt.includes('←')||txt.includes('return')){el.style.display='none';el.remove()}if(el.tagName==='BUTTON'){if(txt.includes('enroll')||txt.includes('add to cart')||txt.includes('buy')){if(c.access_type==='free'){el.textContent='Enroll for Free'}else if(c.access_type==='paid'){el.textContent='Enroll Now - ₹'+c.price}else if(c.access_type==='coming-soon'){el.textContent='Coming Soon';el.disabled=true;el.classList.add('opacity-50','cursor-not-allowed')}else if(c.access_type==='enrollment-closed'){el.textContent='Enrollment Closed';el.disabled=true;el.classList.add('opacity-50','cursor-not-allowed')}el.disabled=true;el.style.pointerEvents='none'}}});const syl=document.querySelector('.course-syllabus, .syllabus');if(syl&&c.sections?.length>0){syl.innerHTML=c.sections.map((s,i)=>'<details class="group rounded-lg border border-gray-200 dark:border-gray-700"><summary class="flex cursor-pointer items-center justify-between p-4 font-medium hover:bg-gray-50 dark:hover:bg-gray-800/50" style="pointer-events:auto;cursor:pointer;"><span>Module '+(i+1)+': '+s.title+'</span><span class="material-symbols-outlined transition-transform group-open:rotate-180">expand_more</span></summary>'+(s.description?'<div class="border-t border-gray-200 p-4 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-400">'+s.description+'</div>':'')+'</details>').join('');const sylSection=syl.closest('section');if(sylSection){const allSections=Array.from(document.querySelectorAll('section'));const sylIndex=allSections.indexOf(sylSection);allSections.forEach((sec,idx)=>{if(idx>sylIndex)sec.style.display='none'})}}})();`;
        ...
        {isLoadingPreview ? (
            ...
            <div className="bg-white rounded-lg shadow-2xl overflow-hidden" style={{ maxWidth: '1440px', margin: '0 auto' }}>
              ...
              <iframe 
                ...
                style={{ height: '1400px', pointerEvents: 'auto', userSelect: 'none' }}
